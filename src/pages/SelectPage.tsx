    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-secondary/50">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-primary">// Select Location</span>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4 tracking-tight">Select State & City</h2>
          ...
          <div className="relative mb-4">
            <input
              ...
              className="w-full max-w-md bg-secondary/50 border border-border rounded-sm px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors"
              maxLength={100}
            />
              ...
              <div className="absolute top-full left-0 w-full max-w-md mt-1 bg-card border border-border rounded-sm z-20 max-h-60 overflow-y-auto">
                {searchResults.map((r, i) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs hover:bg-muted/50 transition-colors flex justify-between"
                  >
                    <span className="text-foreground">{r.city.name}, <span className="text-muted-foreground">{r.state}</span></span>
          ...
          {/* SVG Map */}
          <div className="bg-secondary/20 rounded-sm border border-border p-2" style={{ maxWidth: 500, height: 420 }}>
                ...
                return (
                  ...
                  >
                    ...
                    {(isSelected || isHovered) && (
                      <text x={x} y={y - size - 4} textAnchor="middle" fill="hsl(var(--foreground))" fontSize="7" fontFamily="JetBrains Mono">
                        {state.code} ({solvency})
                      </text>
                    )}
        ...
        <aside className="w-80 shrink-0 border-l border-border overflow-y-auto bg-secondary/10">
          {/* State Dropdown / List */}
          <div className="p-3">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className={state.name === selectedState ? 'text-primary font-semibold' : 'text-foreground'}>
                        {state.name}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-bold ${getSolvencyClass(solvency)}`}>{solvency}</span>
                      <span className="text-muted-foreground text-[9px]">{state.cities.length}c</span>
                    </div>
                  </button>
                );
              })}
                ...
                {selectedStateData.cities.map(city => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-xs font-bold ${getSolvencyClass(city.solvency)}`}>{city.solvency}</span>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-[10px]">
                      <div>
                        <div className="text-muted-foreground">GW Level</div>
                        <div className="text-foreground font-semibold">{city.cgwbDepth}m</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Reservoir</div>
                        <div className="text-foreground font-semibold">{city.cwcStorage}%</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Day Zero</div>
                        <div className="text-foreground font-semibold">{city.dayZeroDays}d</div>
                      </div>
                    </div>
                  </button>
                ))}
